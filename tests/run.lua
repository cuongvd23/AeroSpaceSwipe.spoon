...
test("a short swipe releases blocking after confirmation even if dispatch is pending", function()
	local f, c = fixture()
	f:swipe()
	c.dispatchTimer:stop()
	f:touch({})
	f:advance(0.049)
	equal(c:status().gestureActive, true)
	f:advance(0.002)
...
test("brief empty frames during a held swipe cannot cause a second command", function()
	local f, c = fixture()
	f:swipe(0.52)
	f:advance(0)
	f:finish(1)
	-- The captured failure had an empty frame about 90 ms after recognition,
	-- followed by continued movement from the same four contacts.
	f:advance(0.09)
	f:touch({})
	f:advance(0.006)
	f:touch(frame(0.54))
	...
	for i = 1, 20 do
		f:touch({})
		f:advance(0.04)
		f:touch(frame(i % 2 == 0 and 0.4 or 0.6))
	end
	f:touch({})
	f:advance(0.049)
	f:touch(frame(0.6, nil, { 1, 2, 3 }))
	f:advance(0.01)
	f:touch(frame(0.7))
	equal(c:status().gestureActive, true)
	equal(c:status().pointerTapEnabled, true)
...
test("repeated empty frames confirm release without postponing the deadline", function()
	local f, c = fixture()
	f:swipe()
	f:touch({})
	f:advance(0.03)
	...
	equal(c:status().gestureActive, false)
	equal(c:status().pointerTapEnabled, false)
	equal(c:status().momentumBlocked, true)
	f:finish(1)
	f:touch(frame(0.5))
	f:touch(frame(0.52))
	f:advance(0)
