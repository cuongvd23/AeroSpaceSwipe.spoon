...
test("new identities recover when the release event was missed", function()
	local r = recognizer()
	r:update(frame(0.48))
...
test("reused identities with began phases are a new gesture", function()
	local r = recognizer()
	r:update(frame(0.48))
...
test("lost release expires input blocking without a duplicate switch", function()
	local f, c = fixture()
	f:swipe()
	f:advance(0.51)
	equal(c:status().gestureActive, false)
